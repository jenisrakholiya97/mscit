...
const Sidebar = ({ isOpen = false, onClose }) => {
  const { user, logout, isOwner, isManager, hasPermission } = useAuth();

  const navItems = [
    { label: 'Dashboard', path: '/', icon: LayoutDashboard, perm: null },
    { label: 'Products', path: '/products', icon: Package, perm: null },
    { label: 'POS Billing', path: '/pos', icon: ShoppingCart, perm: 'pos_billing' },
    { label: 'Purchases', path: '/purchases', icon: Truck, perm: 'manage_purchases' },
    { label: 'Inventory Audit', path: '/inventory', icon: Boxes, perm: 'manage_inventory' },
    { label: 'Expiry Alerts', path: '/expiry', icon: AlertTriangle, perm: null },
    { label: 'AI Demand Forecast', path: '/ai-forecast', icon: TrendingUp, perm: null, badge: 'AI' },
    { label: 'AI Smart Reorder', path: '/ai-reorder', icon: Sparkles, perm: 'manage_purchases', badge: 'AI' },
    { label: 'Reports', path: '/reports', icon: FileText, perm: 'view_reports' },
    { label: 'Suppliers', path: '/suppliers', icon: Building2, perm: 'manage_suppliers' },
    { label: 'User & Permissions', path: '/users', icon: Users, perm: 'manage_users' },
  ];

  const filteredItems = navItems.filter(item => {
    if (item.path === '/users') return isOwner;
